...
export class CaslAbilityService {
  ...
  async forRoot(username: string) {
    ...
    this.logger.log('查询出来用户信息:', JSON.stringify({
    ...
    user.roles.forEach((o) => {
      this.logger.log(`遍历角色：id=${o.id}, name=${o.name}, 该角色绑定菜单数量:${o.menus.length}`);
    ...
    this.logger.log('去重后菜单列表：', JSON.stringify(menus.map(m=>({
      id:m.id,
      name:m.name,
      path:m.path,
      acl:m.acl
    ...
    menus.forEach((menu) => {
      ...
      this.logger.log(`菜单[${menu.name}] path=${menu.path} acl原始字符串=${menu.acl} 解析actions:`,actions);
      const entity = getEntities(menu.path);
      this.logger.log(`---> 注册权限 can(${JSON.stringify(actions)}, ${entity})`);
...
// 打印最终生成的全部权限规则
   this.logger.log(`✅CASL最终生成全部权限rules: ${JSON.stringify(ability.rules)}`);
